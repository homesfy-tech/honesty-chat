...
/**
 * PostgreSQL storage for Widget Config
 */
export async function getWidgetConfig(projectId) {
  const result = await query(
    'SELECT * FROM widget_configs WHERE project_id = $1',
    [projectId]
  );
...
export async function updateWidgetConfig(projectId, updates) {
  ...
  const result = await query(
    `UPDATE widget_configs SET ${fields.join(', ')} WHERE project_id = $${paramCount} RETURNING *`,
    values
  );
...
export async function deleteWidgetConfig(projectId) {
  await query('DELETE FROM widget_configs WHERE project_id = $1', [projectId]);
  return true;
}
